...
*/
#ifndef ULSAkmeans_MC_H
#define ULSAkmeans_MC_H
...
template class ULSAOutputToolSet<class ULSAkmeans_MC>;

class ULCS1b;
class ULAGENT;
class ULSAkmeans_MC :public SABASE
{
public:
  ULSAkmeans_MC();
  //Constructor for scenario wise parameter
  ULSAkmeans_MC(FILE *fileReadCursor, int inputTotalNodes, int inputMaxChNum,int inputSAFac,  
               int inOutputControl,
               int isStrucOuput,
...
//-------------------------------------------------------------------//
  ...
  struct compareDis
  {
    const ULSAkmeans_MC& mySA;// data member
    compareDis(const ULSAkmeans_MC& sa) : mySA(sa) {} // constructor
  ...
  bool iniDone;
  bool *aryFlagHRDone; //If it is true means Head Rotate have been done in this structure, starrt from ULSAkmeans_MC
