...
int main(int argc, char* argv[])
{
  ...
  // Control Constant    //
  //---------------------//
  char iniFlag[]="baselineKmedoidMC";
 ...
 // double density= static_cast<double>(totalNodes)*1e6/(3.1415*radius*radius);
  ...
  //ULSAkmeans_MC *toolSA = new ULSAkmeans_MC(fid, totalNodes, maxChNum, SAIter,outputControl, SA_INI_TEMP, alpha, correlationFactor);
  //ULSAkmeans_MC toolSA(fid, totalNodes, maxChNum, SAIter,outputControl, SA_INI_TEMP, alpha, correlationFactor);
  ULSAkmeans_MC toolSA(fid, totalNodes, maxChNum, SAIter,outputControl, isDetailOn,SA_INI_TEMP, alpha, compressionRatio,ipAddr );
