...
MinPowerImageScheduler::~MinPowerImageScheduler()
{

}

double
MinPowerImageScheduler::ScheduleOneSlot( std::vector<int>& vecSupport )
{
  /* NULL */
  return 0.0;
}
...
double
MinPowerImageScheduler::ScheduleOneSlot( std::vector<int>& vecSupport, std::vector<double>& vecPower, const std::vector<double>& vecVariance)
{
...
void 
MinPowerImageScheduler::InitSolution()
{
  ...
  SmartPtr<TMINLP> tminlp = 
    new MinPowerImageMILP( numVariables, numConstraints, numNz_jac_g, numNz_h_lag,
        m_txTimePerSlot, m_tier2NumSlot, m_bandwidthKhz,  
        m_ptrCS, m_ptrMap);
  MinPowerImageMILP* rawPtr = dynamic_cast<MinPowerImageMILP*>(GetRawPtr(tminlp));
  ...
  bonmin.readOptionsFile("MinPowerImageMILP.opt");
