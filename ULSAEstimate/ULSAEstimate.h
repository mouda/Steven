...
*/
#ifndef ULSAEstimate_H
#define ULSAEstimate_H
...
template class ULSAOutputToolSet<class ULSAEstimate>;

class ULCS1b;
class ULAGENT;
class ULSAEstimate :public SABASE
{
public:
  ULSAEstimate();
  //Constructor for scenario wise parameter
  ULSAEstimate(FILE *fileReadCursor, int inputTotalNodes, int inputMaxChNum,int inputSAFac,  
               int inOutputControl,
               int isStrucOuput,
...
//-------------------------------------------------------------------//
  ...
  struct compareDis
  {
    const ULSAEstimate& mySA;// data member
    compareDis(const ULSAEstimate& sa) : mySA(sa) {} // constructor
  ...
  bool iniDone;
  bool *aryFlagHRDone; //If it is true means Head Rotate have been done in this structure, starrt from ULSAEstimate
...
//-------------------------------------------------------------------//
  ...
  void genNeighborhoodMat(vector<vector<int> > &matAvailableCount); 
  double computePcInterference_GivenTarnInJoinI(const int& joinCHIdx, const int& targetCHIdx);
  double computePcInterference(const int& CHIdx);
