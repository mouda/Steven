...
void ULSAEstimate::decideHeadJoining4b(){
    ...
    estimaptePayOff = curPayoff-tmpGainTest;
...
void ULSAEstimate::decideHeadJoining4d() 
{
  int threshold=thresholdd; // To determine the cluster size is large enough 
  // to perform join operation 
  double usepower=powerMax; // No use
 ...
 * @retval   
 */
void ULSAEstimate::genNeighborhoodMat(vector<vector<int> > &matAvailableCount) 
{
...
double ULSAEstimate::computePcInterference_GivenTarnInJoinI(const int& joinCHIdx, 
...
double ULSAEstimate::computePcInterference(const int& CHIdx)
{
  double sumPower = 0.0; 
  for (int i = 0; i < maxChNum; i++) {
    /* tight bound */
