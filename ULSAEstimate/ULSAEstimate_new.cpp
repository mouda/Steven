...
using namespace std;
#include "ULSAEstimate.h"
ULSAEstimate::ULSAEstimate() {};

ULSAEstimate::ULSAEstimate(FILE *fileReadCursor, int inputTotalNodes, int inputMaxChNum,int inputSAFac,  \
                     int inOutputControl,
                     int isStrucOuput,
...
void ULSAEstimate::releaseMemory()
{
...
// @Called: by main
//-------------------------------------------------------------------//
bool ULSAEstimate::setSystem(float inPowerMaxWatt, int inQuantizationBits,double inBandwidthKhz, double inFidelity)
{
...
*/
bool ULSAEstimate::setInitialStucture(char* iniFlag)
{
    iniDone=false;
...
*/
bool ULSAEstimate::setIniStruKmeans()
{
  int retryTimes = 0;
...
bool ULSAEstimate::setIniHeadLimited()
{
...
bool ULSAEstimate::setIniStruDistanceKmedoids() 
{
  int retryTimes = 0;
  float tempHeadX [maxChNum];
  float tempHeadY [maxChNum];
...
bool ULSAEstimate::setIniStruHalfResourceKmedoids()
{
  int retryTimes = 0;
  float tempHeadX [maxChNum];
  float tempHeadY [maxChNum];
...
bool ULSAEstimate::setIniStruFullResourceKmedoids()
{
  int retryTimes = 0;
  float tempHeadX [maxChNum];
  float tempHeadY [maxChNum];
...
void ULSAEstimate::writeStruSingleRound(int round)
{
    FILE *fid;
    char str[50];
...
void ULSAEstimate::writePayoffEachRound_MinResors(int round)
{
    char str[]="ULSAEstimateEachR.txt";
...
void ULSAEstimate::writePayoffEachRound_MinResors_withHead(int round,int head)
{
    char str[]="ULSAEstimateEachR.txt";
    FILE* fid = fopen(str,"a");
...
void ULSAEstimate::do1sttierPowerControlforNext_DataCentric() {
...
void ULSAEstimate::do1sttierPowerControlforBest_DataCentric() {
    //debug_CheckSizeCorrect();
...
void ULSAEstimate::do1sttierPowerControlforTEMP_DataCentric(double &temp1stJoule, double &temp1stMs) {
    for(unsigned int i=0; i<cSystem->vecHeadName.size(); i++) {
        double  groupRedundancy = matrixComputer->computeLog2Det(1.0,cSystem->clusterStru[i]);
        double  clusterInfo=cSystem->vecClusterSize[i]*indEntropy +  groupRedundancy;
        vecClusterHeadBits[i]=clusterInfo;
...
void ULSAEstimate::do1sttierPowerMaxforBest_DataCentric() {
...
void ULSAEstimate::computeUpperResourceNoCodingNoPowerControl()
{
...
void ULSAEstimate::computeBestAvgInterference()
{
...
void ULSAEstimate::computeBestAvgPower()
{
...
double ULSAEstimate::returnComprRatio()
{
...
bool ULSAEstimate::startCool()
{
  ...
  ULSAOutputToolSet<class ULSAEstimate> resultShow;
  flagAnsFound =false;
  //-----------------------------//
...
*/
void ULSAEstimate::addMemberSAIni(int inputHeadIndex, int inputMemberName)
{
...
void ULSAEstimate::do1sttierPowerControlforCur_DataCentric() {
// debug_CheckSizeCorrect();
//  cout<<endl;
...
*/
void ULSAEstimate::coolOnce_minResors()
{
...
*/
 void ULSAEstimate::decideAdd3i_DC_HeadDetMemRan() {
    targetHeadIndex=-1;
    targetNode=-1;
...
*/
void ULSAEstimate::decideDiscard3b()
{
...
void ULSAEstimate::decideDiscard3i_DC_HeadRanNodeDet_CompressionRatio() {
...
void ULSAEstimate::decideHeadRotate2i_DC_HeadRanMemDet()
{
	//----Uniformly choosed
...
void ULSAEstimate::decideHeadJoining4b(){
...
double ULSAEstimate::estimateJoin2ndTierCost(int joinCHIdx, int targetCHIdx){
...
void ULSAEstimate::computeOriInterference_GivenTarInJoinI(
...
void ULSAEstimate::updateJoinEstimatedPower(vector<double> &newPower, 
...
void ULSAEstimate::computeNewInterference_FromNewTarHI(vector<double> &newInterf,
...
void ULSAEstimate::decideIsolate4b(){
...
double ULSAEstimate::estimateIsolate2ndTierGain(int IsoNodeName,int isoCluIndex){
...
void ULSAEstimate::computeOriInterference_GivenIsolate(vector<double> &oriInterf,vector<double> &interfExcept,int isoCluIndex){
    consSol->updateInterference();
    for(int i=0;i<maxChNum;i++){
        if(cSystem->vecHeadName[i]==-1)continue;
        double tempInterf=0;
...
void ULSAEstimate::updateIsolateEstimatedpower(vector<double> &newPower,int IsolName,int isoCluIndex){
    list<list <int> >::iterator it_LiInt=cSystem->listCluMember->begin();
...
void ULSAEstimate::computeNewInterference_FromIsoCluster(vector<double> &newInterf,std::vector<double>&newPower,int IsoName,int isoCluIndex){
    list<list <int> >::iterator it_LiInt=cSystem->listCluMember->begin();
    for(int i=0;i<isoCluIndex;i++)it_LiInt++;
...
*/
void ULSAEstimate::addMemberSA(int inputHeadIndex, int inputMemberName)
{
	cSystem->addMemberCs(inputHeadIndex,inputMemberName,iniDone);
	nodes[inputMemberName].ptrHead = cSystem->returnHeadPtr(inputHeadIndex);
}
/*
   ...
   */
void ULSAEstimate::discardMemberSA(int inputHeadIndex, int inputMemberName)
{
...
/*
   Rotate Member SA
   */
void ULSAEstimate::rotateHeadSA(int inputHeadIndex, int inputMemberName)
{
	rotatedHeadNameLast = cSystem->vecHeadName[inputHeadIndex];
	cSystem->vecHeadName[inputHeadIndex] = inputMemberName;
}

void ULSAEstimate::isolateHeadSA(int isoName,int IsolateCluI, int targetH){
...
void ULSAEstimate::join_fromHeadSA(int JoiningHeadIndex,int targetH){
...
void ULSAEstimate::calculateMatrics_minResors()//Calculate next performance matircs
{
...
/*
   ...
   */
void ULSAEstimate::confirmNeighbor3i()
{
...
void ULSAEstimate::passNext2Cur() {
...
*/
void ULSAEstimate::reverseMoveSA()
{
    //cout<<"reverse"<<endl;
...
/*
  ...
  -reset "aryFlagHRDone"
*/
void ULSAEstimate::confirmStructureChange()
{
...
*/
bool ULSAEstimate::checkBestClusterStructure_DataCentric(int inputRound)
{
    bool curAllServe = (curJEntropy>fidelityRatio*wholeSystemEntopy?true:false);
...
void ULSAEstimate::keepBestStructure()
{
...
*/
int ULSAEstimate::returnClosetNodeIndexInGroup(int tempX,int tempY, vector<int> &inputGroup)
{
...
*/
double ULSAEstimate::returnTransientAveragePower()
{
    list <list<int> >::iterator itlist1=cSystem->listCluMember->begin();
    double accuPower=0;
    int NodeNHeadNum=0;
...
double ULSAEstimate::returnTransientJoule() {
    list <list<int> >::iterator itlist1=cSystem->listCluMember->begin();
    double accuJoule=0;
...
double ULSAEstimate::return1stTotalNcal1stResors_HomoPower() {
    power1st=powerMax;
    double T1=0;
    list <list <int> >::iterator it_LiInt=cSystem->listCluMember->begin();
    for(unsigned int i=0; i<cSystem->vecHeadName.size(); i++,it_LiInt++) {
...
void ULSAEstimate::resetSA3iSystem() {
    iniDone = false;
...
/* Not finishied
void ULSAEstimate::writePayoffEachRound_MinResors(int inputRound)
{
  FILE *fid1;
  fid1=fopen("ULSAEstimate_iterPayoff.txt","a");
