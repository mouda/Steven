...
int main(int argc, char* argv[])
{
  ...
  if(argc!=14)
  {
    ...
    cout<<"Example:Usage: winULSAEstimate 195 10 10 10 180 mapFile/mapFile_uni_195_r500/mapFile_uniR500_N195_2.txt 10000 2 2 3"<<endl;
 ...
 // double density= static_cast<double>(totalNodes)*1e6/(3.1415*radius*radius);
  ...
  //ULSAEstimate *toolSA = new ULSAEstimate(fid, totalNodes, maxChNum, SAIter,outputControl, SA_INI_TEMP, alpha, correlationFactor);
  //ULSAEstimate toolSA(fid, totalNodes, maxChNum, SAIter,outputControl, SA_INI_TEMP, alpha, correlationFactor);
  ULSAEstimate toolSA(fid, totalNodes, maxChNum, SAIter,outputControl, isDetailOn,SA_INI_TEMP, alpha, compressionRatio,ipAddr );
