...
MinPowerImageCluster::MinPowerImageCluster(
    FILE *fileReadCursor, 
    int inputTotalNodes, 
    int inputMaxChNum,
    int inputSAFac,  
...
std::vector<int>
MinPowerImageCluster::GetAllSupStru() const {
...
void MinPowerImageCluster::releaseMemory()
{
...
// @Called: by main
//-------------------------------------------------------------------//
bool MinPowerImageCluster::setSystem(float inPowerMaxWatt, int inQuantizationBits,double inBandwidthKhz, double inFidelity)
{
...
*/
bool MinPowerImageCluster::setInitialStucture(char* iniFlag)
{
    iniDone=false;
...
bool MinPowerImageCluster::setIniStruDistanceKmedoids() 
{
  int retryTimes = 0;
...
bool
MinPowerImageCluster::setIniGraphPartition()
{
  /* read file */
  fstream fpart;
...
bool
MinPowerImageCluster::setIniBanancedModelCluster()
{
  int retryTimes = 0;
  float tempHeadX [maxChNum];
  float tempHeadY [maxChNum];
...
double
MinPowerImageCluster::GetNodeDistance( const int lName, const int rName)
{
  return sqrt( 
...
MinPowerImageCluster::returnComprRatio()
{
    ...
    double sysRedundancyLocal=m_ptrImageSource->computeLog2Det(1.0,tempAry2);
...
bool MinPowerImageCluster::startCool()
{
  begin = clock();
...
*/
void MinPowerImageCluster::addMemberSAIni(int inputHeadIndex, int inputMemberName)
{
...
*/
void MinPowerImageCluster::GetNeighbor1( const int iterSA)
{
  int probAdd = 0;
  int probDiscard = 0;
  if (iterSA < SAIter/5) {
...
void
MinPowerImageCluster::GetNeighbor2( const int iterSA, 
...
double
MinPowerImageCluster::GetPayOff( 
    const vector<double>& sizePenalty, 
    const vector<double>& tier2Penalty, 
    const double& entropyPenalty)
{
 ...
 */
void 
MinPowerImageCluster::decideExchangeNode()
{
  decideDiscard3b();
...
*/
void 
MinPowerImageCluster::decideAddSmallestSize() {
  targetHeadIndex=-1;
  targetNode=-1;
  double curInfo=curSupNum*indEntropy+m_ptrImageSource->computeLog2Det(1.0,cSystem->allSupStru);
...
void 
MinPowerImageCluster::decideAdd3i_DC_HeadDetMemRan() {
  targetHeadIndex=-1;
  targetNode=-1;
  ...
  for(; it_Int!=cSystem->listUnSupport->end(); it_Int++) {
    ...
    double tmpIndInfo=(curSupNum+1)*indEntropy+m_ptrImageSource->computeLog2Det(1.0,cSystem->allSupStru)-curInfo;
...
void 
MinPowerImageCluster::decideAddClosetAddableNode() {
  targetHeadIndex=-1;
  targetNode=-1;
...
void
MinPowerImageCluster::decideAddRandSelectCluster()
{
  targetHeadIndex = -1;
  targetNode = -1;
...
*/
void 
MinPowerImageCluster::decideDiscard3b()
{
...
void 
MinPowerImageCluster::decideDiscard3o()
{
    //Compute the discardable size
    list<list<int> >::iterator itli1 = cSystem->listCluMember->begin();
...
void
MinPowerImageCluster::decideDiscard3f()
{
    //Compute the discardable size
    list<list<int> >::iterator itli1 = cSystem->listCluMember->begin();
    int discardableSize=0;
...
void
MinPowerImageCluster::decideDiscardMinGain()
{
  //Compute the discardable size
...
void 
MinPowerImageCluster::decideHeadRotate2i_DC_HeadRanMemDet()
{
  //----Uniformly choosed
...
void 
MinPowerImageCluster::decideHeadJoining4b(){
...
double
MinPowerImageCluster::GetClusterEntropy(const int chIdx)
{
...
double
MinPowerImageCluster::GetJoinHeadEntropy(const int lChIdx, const int rChIdx)
{
...
bool
MinPowerImageCluster::CheckTwoLinkFeasible(const int lChName, const int lMemberName, const int rChName, const int rMemberName)
{
  double Gamma = 1.0;
...
bool
MinPowerImageCluster::CheckLinkFeasible(const int chName, const int name)
{
  std::list<std::list<int> >::const_iterator iterRow = cSystem->listCluMember->begin();
...
bool
MinPowerImageCluster::CheckAllFeasible()
{
  std::list<std::list<int> >::const_iterator iterRow =  cSystem->listCluMember->begin();
  for (int idx = 0; iterRow != cSystem->listCluMember->end(); ++iterRow, ++idx) {
    std::list<int>::const_iterator iterCol = iterRow->begin();
...
double
MinPowerImageCluster::GetTier2ExpectPower(const int Name, const int chName)
{
...
bool
MinPowerImageCluster::CheckTier2Feasible()
{
  list<list<int> >::const_iterator iterRow = cSystem->listCluMember->begin();  
  for (int chIdx = 0; iterRow != cSystem->listCluMember->end(); ++iterRow, ++chIdx) {
    if (cSystem->vecHeadName.at(chIdx) == -1 ) continue; 
...
double
MinPowerImageCluster::GetSizePenalty( const vector<double>& sizePenalty)
{
...
double
MinPowerImageCluster::GetTier2Penalty( const vector<double>& tier2Penalty )
{
  double penalty = 0.0;
  list<list<int> >::const_iterator iterRow = cSystem->listCluMember->begin();  
  for (int chIdx = 0; iterRow != cSystem->listCluMember->end(); ++iterRow, ++chIdx) {
...
void MinPowerImageCluster::decideIsolate4b(){
...
*/
void MinPowerImageCluster::addMemberSA(int inputHeadIndex, int inputMemberName)
{
  cSystem->addMemberCs(inputHeadIndex,inputMemberName,iniDone);
  nodes[inputMemberName].ptrHead = cSystem->returnHeadPtr(inputHeadIndex);
}
/*
   ...
   */
void MinPowerImageCluster::discardMemberSA(int inputHeadIndex, int inputMemberName)
{
...
/*
   Rotate Member SA
   */
void MinPowerImageCluster::rotateHeadSA(int inputHeadIndex, int inputMemberName)
{
	rotatedHeadNameLast = cSystem->vecHeadName[inputHeadIndex];
	cSystem->vecHeadName[inputHeadIndex] = inputMemberName;
}

void MinPowerImageCluster::isolateHeadSA(int isoName,int IsolateCluI, int targetH){
...
void MinPowerImageCluster::join_fromHeadSA(int JoiningHeadIndex,int targetH){
...
void MinPowerImageCluster::calculateMatrics_minResors(//Calculate next performance matircs
    const vector<double>& sizePenalty, 
    const vector<double>& tier2Penalty, 
    const double& entropyPenalty)
{
...
void
MinPowerImageCluster::ConfirmNeighbor2(
...
void 
MinPowerImageCluster::passNext2Cur() 
{
...
*/
void MinPowerImageCluster::reverseMoveSA()
{
    //cout<<"reverse"<<endl;
...
/*
  ...
  -reset "aryFlagHRDone"
*/
void MinPowerImageCluster::confirmStructureChange()
{
...
*/
bool MinPowerImageCluster::checkBestClusterStructure_DataCentric(int inputRound)
{
...
void MinPowerImageCluster::keepBestStructure()
{
...
*/
int MinPowerImageCluster::returnClosetNodeIndexInGroup(int tempX,int tempY, std::vector<int> &inputGroup)
{
...
double MinPowerImageCluster::returnTransientJoule() {
...
void MinPowerImageCluster::resetSA3iSystem() {
    iniDone = false;
