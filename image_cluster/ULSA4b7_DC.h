...
class ULCS1b;
class ULAGENT;
class ULSA4b7_DC :public SABASE
{
public:
  //Constructor for scenario wise parameter
  ULSA4b7_DC(FILE *fileReadCursor, int inputTotalNodes, int inputMaxChNum,int inputSAFac,  
               int inOutputControl,
               int isStrucOuput,
...
//-------------------------------------------------------------------//
  ...
  struct compareDis
  {
    const ULSA4b7_DC& mySA;// data member
    compareDis(const ULSA4b7_DC& sa) : mySA(sa) {} // constructor
  ...
  bool iniDone;
  bool *aryFlagHRDone; //If it is true means Head Rotate have been done in this structure, starrt from ULSA4b7_DC
...
//-------------------------------------------------------------------//
  ...
  void coolOnce_minResors(const int );
