#include "baselineImageScheduler.h"

using Ipopt::Index;

BaselineImageScheduler::BaselineImageScheduler( const double txTime, 
...
BaselineImageScheduler::~BaselineImageScheduler()
{

}

double
BaselineImageScheduler::ScheduleOneSlot( std::vector<int>& vecSupport )
{
  /* NULL */
  return 0.0;
}
...
double
BaselineImageScheduler::ScheduleOneSlot( std::vector<int>& vecSupport, std::vector<double>& vecPower, const std::vector<double>& vecVariance)
{
...
void 
BaselineImageScheduler::InitSolution()
{
  ...
  SmartPtr<TMINLP> tminlp = 
    new BaselineImageMILP( numVariables, numConstraints, numNz_jac_g, numNz_h_lag,
        m_txTimePerSlot, m_tier2NumSlot, m_bandwidthKhz,  
        m_ptrCS, m_ptrMap);
  BaselineImageMILP* rawPtr = dynamic_cast<BaselineImageMILP*>(GetRawPtr(tminlp));
  ...
  bonmin.readOptionsFile("BaselineImageMILP.opt");
