...
using Ipopt::Index;
MaxSumRate::MaxSumRate( const double txTime, 
...
double
MaxSumRate::ScheduleOneSlot( std::vector<int>& vecSupport )
{
...
double
MaxSumRate::ScheduleOneSlot( std::vector<int>& vecSupport, std::vector<double>& vecPower, const std::vector<double>& vecVariance)
{
...
int
MaxSumRate::SolverHook(std::vector<int>& vecSupport, Eigen::MatrixXd& matSigma)
{
  Eigen::IOFormat CleanFmt(2, 0, " ", "\n", "", ";");
  ...
  SumRateIP* rawPtr = dynamic_cast<SumRateIP*>(GetRawPtr(tminlp));
...
double
MaxSumRate::OmegaValue(const int nodeName)
{
