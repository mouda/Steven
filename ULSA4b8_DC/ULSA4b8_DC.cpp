...
// @Called: by main
//-------------------------------------------------------------------//
bool ULSA4b8_DC::setSystem(float inPowerMaxWatt, int inQuantizationBits,double inBandwidthKhz, double inFidelity)
{
...
*/
bool ULSA4b8_DC::setIniStruKmeans()
{
  int retryTimes = 0;
...
bool ULSA4b8_DC::setIniHeadLimited()
{
...
bool ULSA4b8_DC::setIniStruDistanceKmedoids() 
{
  int retryTimes = 0;
  float tempHeadX [maxChNum];
  float tempHeadY [maxChNum];
...
bool ULSA4b8_DC::setIniStruHalfResourceKmedoids()
{
  int retryTimes = 0;
  float tempHeadX [maxChNum];
  float tempHeadY [maxChNum];
...
bool ULSA4b8_DC::setIniStruFullResourceKmedoids()
{
  int retryTimes = 0;
  float tempHeadX [maxChNum];
  float tempHeadY [maxChNum];
...
void ULSA4b8_DC::writeStruSingleRound(int round)
{
    FILE *fid;
    char str[50];
...
void ULSA4b8_DC::writePayoffEachRound_MinResors(int round)
{
    char str[]="ULSA4b8_DCEachR.txt";
...
void ULSA4b8_DC::writePayoffEachRound_MinResors_withHead(int round,int head)
{
    char str[]="ULSA4b8_DCEachR.txt";
    FILE* fid = fopen(str,"a");
...
void ULSA4b8_DC::debug_CheckSizeCorrect() {
...
void ULSA4b8_DC::computeBestTRR_DataCentric()
{
...
void ULSA4b8_DC::computeUpperResourceNoCodingNoPowerControl()
{
...
void ULSA4b8_DC::computeBestAvgInterference()
{
...
void ULSA4b8_DC::computeBestAvgPower()
{
...
double ULSA4b8_DC::returnComprRatio()
{
...
bool ULSA4b8_DC::startCool()
{
  ...
  ULSAOutputToolSet<class ULSA4b8_DC> resultShow;
  flagAnsFound =false;
  //-----------------------------//
  ...
  if(bestFeasibleJEntropy>=(wholeSystemEntopy*fidelityRatio)) {
    ...
    sprintf(str3,"tmpAll/ULSA4b8_All_N%d_BW%.1fPW%.3f_FR%.2f_r%.1f.%s.txt",totalNodes,bandwidthKhz,powerMax,fidelityRatio,radius, strIpAddr.c_str());
    resultShow.writePeformance_MinResors_with2ndPowerControl_4b(str3,*this, best2nd_ms, fidelityRatio,bestChNum);
    sprintf(str3,"tmpAll/ULSA4b8_Cluster_N%d_BW%.1fPW%.3f_FR%.2f_r%.1f.%s.txt",totalNodes,bandwidthKhz,powerMax,fidelityRatio,radius, strIpAddr.c_str());
...
*/
void ULSA4b8_DC::addMemberSAIni(int inputHeadIndex, int inputMemberName)
{
...
void ULSA4b8_DC::do1sttierPowerControlforCur_DataCentric() {
...
*/
void ULSA4b8_DC::coolOnce_minResors()
{
...
*/
 void ULSA4b8_DC::decideAdd3i_DC_HeadDetMemRan() {
    targetHeadIndex=-1;
    targetNode=-1;
...
*/
void ULSA4b8_DC::decideDiscard3b()
{
...
void ULSA4b8_DC::decideDiscard3i_DC_HeadRanNodeDet_CompressionRatio() {
...
void ULSA4b8_DC::decideHeadRotate2i_DC_HeadRanMemDet()
{
	//----Uniformly choosed
...
void ULSA4b8_DC::decideHeadJoining4b(){
...
double ULSA4b8_DC::estimateJoin2ndTierCost(int joinCHIdx, int targetCHIdx){
...
void ULSA4b8_DC::computeOriInterference_GivenTarInJoinI(
...
void ULSA4b8_DC::updateJoinEstimatedPower(vector<double> &newPower, 
...
void ULSA4b8_DC::computeNewInterference_FromNewTarHI(vector<double> &newInterf,
...
void ULSA4b8_DC::decideIsolate4b(){
...
double ULSA4b8_DC::estimateIsolate2ndTierGain(int IsoNodeName,int isoCluIndex){
...
void ULSA4b8_DC::computeOriInterference_GivenIsolate(vector<double> &oriInterf,vector<double> &interfExcept,int isoCluIndex){
    consSol->updateInterference();
    for(int i=0;i<maxChNum;i++){
        if(cSystem->vecHeadName[i]==-1)continue;
        double tempInterf=0;
...
void ULSA4b8_DC::updateIsolateEstimatedpower(vector<double> &newPower,int IsolName,int isoCluIndex){
    list<list <int> >::iterator it_LiInt=cSystem->listCluMember->begin();
...
void ULSA4b8_DC::computeNewInterference_FromIsoCluster(vector<double> &newInterf,std::vector<double>&newPower,int IsoName,int isoCluIndex){
    list<list <int> >::iterator it_LiInt=cSystem->listCluMember->begin();
    for(int i=0;i<isoCluIndex;i++)it_LiInt++;
...
*/
void ULSA4b8_DC::addMemberSA(int inputHeadIndex, int inputMemberName)
{
	cSystem->addMemberCs(inputHeadIndex,inputMemberName,iniDone);
	nodes[inputMemberName].ptrHead = cSystem->returnHeadPtr(inputHeadIndex);
}
/*
   ...
   */
void ULSA4b8_DC::discardMemberSA(int inputHeadIndex, int inputMemberName)
{
...
/*
   Rotate Member SA
   */
void ULSA4b8_DC::rotateHeadSA(int inputHeadIndex, int inputMemberName)
{
	rotatedHeadNameLast = cSystem->vecHeadName[inputHeadIndex];
	cSystem->vecHeadName[inputHeadIndex] = inputMemberName;
}

void ULSA4b8_DC::isolateHeadSA(int isoName,int IsolateCluI, int targetH){
...
void ULSA4b8_DC::join_fromHeadSA(int JoiningHeadIndex,int targetH){
...
void ULSA4b8_DC::calculateMatrics_minResors()//Calculate next performance matircs
{
...
/*
   ...
   */
void ULSA4b8_DC::confirmNeighbor3i()
{
...
/*
  ...
  -reset "aryFlagHRDone"
*/
void ULSA4b8_DC::confirmStructureChange()
{
...
*/
bool ULSA4b8_DC::checkBestClusterStructure_DataCentric(int inputRound)
{
    bool curAllServe = (curJEntropy>fidelityRatio*wholeSystemEntopy?true:false);
...
void ULSA4b8_DC::keepBestStructure()
{
...
*/
int ULSA4b8_DC::returnClosetNodeIndexInGroup(int tempX,int tempY, vector<int> &inputGroup)
{
...
*/
double ULSA4b8_DC::returnTransientAveragePower()
{
    list <list<int> >::iterator itlist1=cSystem->listCluMember->begin();
    double accuPower=0;
    int NodeNHeadNum=0;
...
double ULSA4b8_DC::returnTransientJoule() {
    list <list<int> >::iterator itlist1=cSystem->listCluMember->begin();
    double accuJoule=0;
...
double ULSA4b8_DC::return1stTotalNcal1stResors_HomoPower() {
    power1st=powerMax;
    double T1=0;
    list <list <int> >::iterator it_LiInt=cSystem->listCluMember->begin();
    for(unsigned int i=0; i<cSystem->vecHeadName.size(); i++,it_LiInt++) {
...
void ULSA4b8_DC::resetSA3iSystem() {
    iniDone = false;
