...
*/
#ifndef ULSA_DC_Estimate_H
#define ULSA_DC_Estimate_H
...
template class ULSAOutputToolSet<class ULSA_DC_Estimate>;

class ULCS1b;
class ULAGENT;
class ULSA_DC_Estimate :public SABASE
{
public:
  ULSA_DC_Estimate();
  //Constructor for scenario wise parameter
  ULSA_DC_Estimate(FILE *fileReadCursor, int inputTotalNodes, int inputMaxChNum,int inputSAFac,  \
...
//-------------------------------------------------------------------//
  ...
  struct compareDis
  {
    const ULSA_DC_Estimate& mySA;// data member
    compareDis(const ULSA_DC_Estimate& sa) : mySA(sa) {} // constructor
  ...
  bool iniDone;
  bool *aryFlagHRDone; //If it is true means Head Rotate have been done in this structure, starrt from ULSA_DC_Estimate
