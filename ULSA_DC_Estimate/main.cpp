...
int main(int argc, char* argv[])
{
  ...
  if(argc!=13)
  {
    ...
    cout<<"Example:Usage: winULSA_DC_Estimate 195 10 10 10 180 mapFile/mapFile_uni_195_r500/mapFile_uniR500_N195_2.txt 10000 2 2 3 0"<<endl;
 ...
 // double density= static_cast<double>(totalNodes)*1e6/(3.1415*radius*radius);
  ...
  //ULSA_DC_Estimate *toolSA = new ULSA_DC_Estimate(fid, totalNodes, maxChNum, SAIter,outputControl, SA_INI_TEMP, alpha, correlationFactor);
  //ULSA_DC_Estimate toolSA(fid, totalNodes, maxChNum, SAIter,outputControl, SA_INI_TEMP, alpha, correlationFactor);
  ULSA_DC_Estimate toolSA(fid, totalNodes, maxChNum, SAIter, outputControl, 
