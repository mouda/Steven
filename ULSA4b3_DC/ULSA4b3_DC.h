...
class ULCS1b;
class ULAGENT;
class ULSA4b3_DC :public SABASE
{
public:
  ULSA4b3_DC();
  //Constructor for scenario wise parameter
  ULSA4b3_DC(FILE *fileReadCursor, int inputTotalNodes, int inputMaxChNum,int inputSAFac,  \
...
//-------------------------------------------------------------------//
  ...
  struct compareDis
  {
    const ULSA4b3_DC& mySA;// data member
    compareDis(const ULSA4b3_DC& sa) : mySA(sa) {} // constructor
  ...
  bool iniDone;
  bool *aryFlagHRDone; //If it is true means Head Rotate have been done in this structure, starrt from ULSA4b3_DC
