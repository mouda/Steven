...
int main(int argc, char* argv[])
{
  ...
  if(argc!=14)
  {
    ...
    cout<<"Example:Usage: winULSA4b7_DC 195 10 10 10 180 mapFile/mapFile_uni_195_r500/mapFile_uniR500_N195_2.txt 10000 2 2 3"<<endl;
  ...
  // Control Constant    //
  //---------------------//
  char iniFlag[]="kmedoids_resource_half";
 ...
 // double density= static_cast<double>(totalNodes)*1e6/(3.1415*radius*radius);
  ...
  //ULSA4b7_DC *toolSA = new ULSA4b7_DC(fid, totalNodes, maxChNum, SAIter,outputControl, SA_INI_TEMP, alpha, correlationFactor);
  //ULSA4b7_DC toolSA(fid, totalNodes, maxChNum, SAIter,outputControl, SA_INI_TEMP, alpha, correlationFactor);
  ULSA4b7_DC toolSA(fid, totalNodes, maxChNum, SAIter,outputControl, isDetailOn,SA_INI_TEMP, alpha, compressionRatio,ipAddr );
