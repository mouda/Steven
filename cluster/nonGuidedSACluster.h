...
*/
#ifndef NONGUIDEDSACLUSTER_H
#define NONGUIDEDSACLUSTER_H
...
extern bool pairCompare(const std::pair<int,double>& lPair, const std::pair<int,double>& rPair);

class ULCS1b;
class ULAGENT;
class NonGuidedSACluster :public SABASE
{
public:
...
//-------------------------------------------------------------------//
  ...
  struct compareDis
  {
    const NonGuidedSACluster& mySA;// data member
    compareDis(const NonGuidedSACluster& sa) : mySA(sa) {} // constructor
  ...
  bool iniDone;
  bool *aryFlagHRDone; //If it is true means Head Rotate have been done in this structure, starrt from NonGuidedSACluster
