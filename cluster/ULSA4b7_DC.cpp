...
void ULSA4b7_DC::writePayoffEachRound_MinResors(int round)
{
    ...
    fprintf(fid,"%f %f %f %f %f %d\n", curJEntropy/wholeSystemEntopy,static_cast<double>(curSupNum)/static_cast<double>(totalNodes), m_curPayoff, cur1st_ms, cur2nd_ms,  round);
    fclose(fid);
...
void ULSA4b7_DC::writePayoffEachRound_MinResors_withHead(int round,int head)
{
    char str[]="ULSA4b7_DCEachR.txt";
    FILE* fid = fopen(str,"a");
    fprintf(fid,"%f %f %f %f %f %d %d\n", curJEntropy/wholeSystemEntopy,static_cast<double>(curSupNum)/static_cast<double>(totalNodes), m_curPayoff, cur1st_ms, cur2nd_ms,  round, head);
    fclose(fid);
...
bool ULSA4b7_DC::startCool()
{
  ...
  m_curPayoff=cur1st_ms+cur2nd_ms;
...
*/
void ULSA4b7_DC::coolOnce_minResors(const int iterSA)
{
...
//  int probDiscard = 0;
  ...
  int probHeadRotate = (((curSupNum>2*maxChNum)&&checkRotateble) ?5000:0);//Don't do head rotate if there are only a few nodes
...
void
ULSA4b7_DC::decideAddRandSelectCluster()
{
  targetHeadIndex = -1;
  targetNode = -1;
  list<list<int> >::iterator itli1 = cSystem->listCluMember->begin();
...
void ULSA4b7_DC::decideHeadJoining4b(){
    ...
    if (isDetailOutputOn) {
      ...
      cout << "with estimate payoff=" << m_curPayoff-tmpGainTest << endl;
      char str[500]; 
