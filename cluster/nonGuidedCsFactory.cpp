...
NonGuidedCsFactory::NonGuidedCsFactory( Map const * const myMap, 
...
NonGuidedCsFactory::~NonGuidedCsFactory()
{
  fclose(m_fid);
  if (m_ptrToolSA != NULL) {
    delete m_ptrToolSA;
...
ClusterStructure*
NonGuidedCsFactory::CreateClusterStructure()
{
  double SAIter = 20000;
  ...
  m_ptrToolSA = new NonGuidedSACluster(
      m_fid, 
...
bool
NonGuidedCsFactory::Kmedoid( std::vector<int>& vecHeadNames, std::list<std::list<int> >& listCluMembers )
{
  int retryTimes = 0;
