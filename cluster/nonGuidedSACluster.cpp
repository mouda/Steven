...
std::vector<int>
NonGuidedSACluster::GetAllSupStru() const {
...
void NonGuidedSACluster::releaseMemory()
{
...
// @Called: by main
//-------------------------------------------------------------------//
bool NonGuidedSACluster::setSystem(float inPowerMaxWatt, int inQuantizationBits,double inBandwidthKhz, double inFidelity)
{
...
*/
bool NonGuidedSACluster::setInitialStucture(char* iniFlag)
{
    iniDone=false;
...
*/
bool NonGuidedSACluster::setIniStruKmeans()
{
  int retryTimes = 0;
...
bool NonGuidedSACluster::setIniStruDistanceKmedoids() 
{
  int retryTimes = 0;
  float tempHeadX [maxChNum];
  float tempHeadY [maxChNum];
...
bool
NonGuidedSACluster::setIniGraphPartition()
{
  /* read file */
  fstream fpart;
...
bool
NonGuidedSACluster::setIniBanancedModelCluster()
{
  int retryTimes = 0;
  float tempHeadX [maxChNum];
  float tempHeadY [maxChNum];
...
double
NonGuidedSACluster::GetNodeDistance( const int lName, const int rName)
{
  return sqrt( 
...
double 
NonGuidedSACluster::returnComprRatio()
{
...
bool NonGuidedSACluster::startCool()
{
  begin = clock();
...
*/
void NonGuidedSACluster::addMemberSAIni(int inputHeadIndex, int inputMemberName)
{
...
*/
void NonGuidedSACluster::coolOnce_minResors( const int iterSA)
{
  int probAdd = 0;
  int probDiscard = 0;
  if (iterSA < SAIter/5) {
 ...
 */
void 
NonGuidedSACluster::decideExchangeNode()
{
  decideDiscard3b();
...
*/
void 
NonGuidedSACluster::decideAddSmallestSize() {
  targetHeadIndex=-1;
  targetNode=-1;
...
void 
NonGuidedSACluster::decideAdd3i_DC_HeadDetMemRan() {
  targetHeadIndex=-1;
  targetNode=-1;
...
void 
NonGuidedSACluster::decideAddClosetAddableNode() {
  targetHeadIndex=-1;
  targetNode=-1;
...
void
NonGuidedSACluster::decideAddRandSelectCluster()
{
  targetHeadIndex = -1;
  targetNode = -1;
...
*/
void 
NonGuidedSACluster::decideDiscard3b()
{
...
void 
NonGuidedSACluster::decideDiscard3o()
{
    //Compute the discardable size
    list<list<int> >::iterator itli1 = cSystem->listCluMember->begin();
...
void
NonGuidedSACluster::decideDiscard3f()
{
    //Compute the discardable size
    list<list<int> >::iterator itli1 = cSystem->listCluMember->begin();
    int discardableSize=0;
...
void
NonGuidedSACluster::decideDiscardMinGain()
{
  //Compute the discardable size
...
void 
NonGuidedSACluster::decideHeadRotate2i_DC_HeadRanMemDet()
{
  //----Uniformly choosed
...
void 
NonGuidedSACluster::decideHeadJoining4b(){
...
double
NonGuidedSACluster::GetClusterEntropy(const int chIdx)
{
...
double
NonGuidedSACluster::GetJoinHeadEntropy(const int lChIdx, const int rChIdx)
{
...
bool
NonGuidedSACluster::CheckTwoLinkFeasible(const int lChName, const int lMemberName, const int rChName, const int rMemberName)
{
  double Gamma = 1.0;
...
bool
NonGuidedSACluster::CheckLinkFeasible(const int chName, const int name)
{
  std::list<std::list<int> >::const_iterator iterRow = cSystem->listCluMember->begin();
...
bool
NonGuidedSACluster::CheckAllFeasible()
{
  std::list<std::list<int> >::const_iterator iterRow =  cSystem->listCluMember->begin();
  for (int idx = 0; iterRow != cSystem->listCluMember->end(); ++iterRow, ++idx) {
    std::list<int>::const_iterator iterCol = iterRow->begin();
...
void NonGuidedSACluster::decideIsolate4b(){
...
*/
void NonGuidedSACluster::addMemberSA(int inputHeadIndex, int inputMemberName)
{
  cSystem->addMemberCs(inputHeadIndex,inputMemberName,iniDone);
  nodes[inputMemberName].ptrHead = cSystem->returnHeadPtr(inputHeadIndex);
}
/*
   ...
   */
void NonGuidedSACluster::discardMemberSA(int inputHeadIndex, int inputMemberName)
{
...
/*
   Rotate Member SA
   */
void NonGuidedSACluster::rotateHeadSA(int inputHeadIndex, int inputMemberName)
{
	rotatedHeadNameLast = cSystem->vecHeadName[inputHeadIndex];
	cSystem->vecHeadName[inputHeadIndex] = inputMemberName;
}

void NonGuidedSACluster::isolateHeadSA(int isoName,int IsolateCluI, int targetH){
...
void NonGuidedSACluster::join_fromHeadSA(int JoiningHeadIndex,int targetH){
...
void NonGuidedSACluster::calculateMatrics_minResors()//Calculate next performance matircs
{
...
void NonGuidedSACluster::passNext2Cur() {
...
*/
void NonGuidedSACluster::reverseMoveSA()
{
    //cout<<"reverse"<<endl;
...
/*
  ...
  -reset "aryFlagHRDone"
*/
void NonGuidedSACluster::confirmStructureChange()
{
...
void NonGuidedSACluster::keepBestStructure()
{
...
*/
int NonGuidedSACluster::returnClosetNodeIndexInGroup(int tempX,int tempY, std::vector<int> &inputGroup)
{
...
double NonGuidedSACluster::returnTransientJoule() {
...
void NonGuidedSACluster::resetSA3iSystem() {
    iniDone = false;
