...
BaselineImageCsFactory::~BaselineImageCsFactory()
{
  fclose(m_fid);
  if (m_ptrToolSA != NULL) {
    delete m_ptrToolSA;
...
BaselineImageCsFactory::CreateClusterStructure()
{
  ...
  m_ptrToolSA = new BaselineImageCluster(
      m_fid, 
...
bool
BaselineImageCsFactory::Kmedoid( std::vector<int>& vecHeadNames, std::list<std::list<int> >& listCluMembers )
{
  int retryTimes = 0;
