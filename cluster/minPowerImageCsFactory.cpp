...
MinPowerImageCsFactory::~MinPowerImageCsFactory()
{
  fclose(m_fid);
  if (m_ptrToolSA != NULL) {
    delete m_ptrToolSA;
...
MinPowerImageCsFactory::CreateClusterStructure()
{
  ...
  m_ptrToolSA = new MinPowerImageCluster(
      m_fid, 
...
bool
MinPowerImageCsFactory::Kmedoid( std::vector<int>& vecHeadNames, std::list<std::list<int> >& listCluMembers )
{
  int retryTimes = 0;
