...
BaselineImageCluster::BaselineImageCluster(
    FILE *fileReadCursor, 
    int inputTotalNodes, 
    int inputMaxChNum,
    int inputSAFac,  
...
std::vector<int>
BaselineImageCluster::GetAllSupStru() const {
...
void BaselineImageCluster::releaseMemory()
{
...
*/
bool BaselineImageCluster::setInitialStucture(char* iniFlag)
{
    iniDone=false;
...
*/
bool BaselineImageCluster::setIniStruKmeans()
{
  int retryTimes = 0;
...
bool BaselineImageCluster::setIniStruDistanceKmedoids() 
{
  int retryTimes = 0;
...
bool
BaselineImageCluster::setIniGraphPartition()
{
  /* read file */
  fstream fpart;
...
bool
BaselineImageCluster::setIniBanancedModelCluster()
{
  int retryTimes = 0;
  float tempHeadX [maxChNum];
  float tempHeadY [maxChNum];
...
double
BaselineImageCluster::GetNodeDistance( const int lName, const int rName)
{
  return sqrt( 
...
int
BaselineImageCluster::GetSupportNodes()
{
  int support = 0;
...
double 
BaselineImageCluster::returnComprRatio()
{
...
*/
void BaselineImageCluster::addMemberSAIni(int inputHeadIndex, int inputMemberName)
{
...
*/
void BaselineImageCluster::GetNeighbor1( const int iterSA)
{
  int probAdd = 0;
  int probDiscard = 0;
  if (iterSA < SAIter/5) {
...
void
BaselineImageCluster::GetNeighbor2( const int iterSA, 
...
double
BaselineImageCluster::GetPayOff( 
    const vector<double>& sizePenalty, 
    const vector<double>& tier2Penalty, 
    const double& entropyPenalty)
{
 ...
 */
void 
BaselineImageCluster::decideExchangeNode()
{
  decideDiscard3b();
...
*/
void 
BaselineImageCluster::decideAddSmallestSize() {
  targetHeadIndex=-1;
  targetNode=-1;
...
void 
BaselineImageCluster::decideAdd3i_DC_HeadDetMemRan() {
  targetHeadIndex=-1;
  targetNode=-1;
...
void 
BaselineImageCluster::decideAddClosetAddableNode() {
  targetHeadIndex=-1;
  targetNode=-1;
...
void
BaselineImageCluster::decideAddRandSelectCluster()
{
  targetHeadIndex = -1;
  targetNode = -1;
...
*/
void 
BaselineImageCluster::decideDiscard3b()
{
...
void 
BaselineImageCluster::decideDiscard3o()
{
    //Compute the discardable size
    list<list<int> >::iterator itli1 = cSystem->listCluMember->begin();
...
void 
BaselineImageCluster::decideHeadRotate2i_DC_HeadRanMemDet()
{
  //----Uniformly choosed
...
void 
BaselineImageCluster::decideHeadJoining4b(){
...
double
BaselineImageCluster::GetClusterEntropy(const int chIdx)
{
...
double
BaselineImageCluster::GetJoinHeadEntropy(const int lChIdx, const int rChIdx)
{
...
bool
BaselineImageCluster::CheckTier2Feasible()
{
  list<list<int> >::const_iterator iterRow = cSystem->listCluMember->begin();  
  for (int chIdx = 0; iterRow != cSystem->listCluMember->end(); ++iterRow, ++chIdx) {
    if (cSystem->vecHeadName.at(chIdx) == -1 ) continue; 
...
double
BaselineImageCluster::GetSizePenalty( const vector<double>& sizePenalty)
{
...
double
BaselineImageCluster::GetTier2Penalty( const vector<double>& tier2Penalty )
{
  double penalty = 0.0;
  list<list<int> >::const_iterator iterRow = cSystem->listCluMember->begin();  
  for (int chIdx = 0; iterRow != cSystem->listCluMember->end(); ++iterRow, ++chIdx) {
...
double
BaselineImageCluster::GetEntropyPenalty(const double entropyPenalty)
{
  double tmpJEntropy = nextSupNum*indEntropy;
  if (fidelityRatio*wholeSystemEntopy - tmpJEntropy < 0) {
    return 0.0;
...
void BaselineImageCluster::decideIsolate4b(){
...
*/
void BaselineImageCluster::addMemberSA(int inputHeadIndex, int inputMemberName)
{
  cSystem->addMemberCs(inputHeadIndex,inputMemberName,iniDone);
  nodes[inputMemberName].ptrHead = cSystem->returnHeadPtr(inputHeadIndex);
}
/*
   ...
   */
void BaselineImageCluster::discardMemberSA(int inputHeadIndex, int inputMemberName)
{
...
/*
   Rotate Member SA
   */
void BaselineImageCluster::rotateHeadSA(int inputHeadIndex, int inputMemberName)
{
	rotatedHeadNameLast = cSystem->vecHeadName[inputHeadIndex];
	cSystem->vecHeadName[inputHeadIndex] = inputMemberName;
}

void BaselineImageCluster::isolateHeadSA(int isoName,int IsolateCluI, int targetH){
...
void BaselineImageCluster::join_fromHeadSA(int JoiningHeadIndex,int targetH){
...
void BaselineImageCluster::calculateMatrics_minResors(//Calculate next performance matircs
    const vector<double>& sizePenalty, 
    const vector<double>& tier2Penalty, 
    const double& entropyPenalty)
{
...
/*
   ...
   */
void BaselineImageCluster::ConfirmNeighbor1()
{
...
void
BaselineImageCluster::ConfirmNeighbor2(
...
void 
BaselineImageCluster::passNext2Cur() 
{
...
*/
void BaselineImageCluster::reverseMoveSA()
{
    //cout<<"reverse"<<endl;
...
/*
  ...
  -reset "aryFlagHRDone"
*/
void BaselineImageCluster::confirmStructureChange()
{
...
*/
bool BaselineImageCluster::checkBestClusterStructure_DataCentric(int inputRound)
{
...
void BaselineImageCluster::keepBestStructure()
{
...
*/
int BaselineImageCluster::returnClosetNodeIndexInGroup(int tempX,int tempY, std::vector<int> &inputGroup)
{
...
double BaselineImageCluster::returnTransientJoule() {
...
void BaselineImageCluster::resetSA3iSystem() {
    iniDone = false;
