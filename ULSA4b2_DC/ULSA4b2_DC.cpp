...
bool ULSA4b2_DC::startCool()
{
    ...
    if(bestFeasibleJEntropy>=(wholeSystemEntopy*fidelityRatio)) {
        ...
        sprintf(str3,"ULSA4b2_All_N%d_BW%.1fPW%.3f_FR%.2f_r%.1f.txt",totalNodes,bandwidthKhz,powerMax,fidelityRatio,radius);
        resultShow.writePeformance_MinResors_with2ndPowerControl_4b(str3,*this, best2nd_ms, fidelityRatio,bestChNum);
        sprintf(str3,"ULSA4b2_Cluster_N%d_BW%.1fPW%.3f_FR%.2f_r%.1f.txt",totalNodes,bandwidthKhz,powerMax,fidelityRatio,radius);
