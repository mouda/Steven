...
template class ULSAOutputToolSet<class ULSA4b6_DC>;

class ULCS1b;
class ULAGENT;
class ULSA4b6_DC :public SABASE
{
public:
  ULSA4b6_DC();
  //Constructor for scenario wise parameter
  ULSA4b6_DC(FILE *fileReadCursor, int inputTotalNodes, int inputMaxChNum,int inputSAFac,  
               int inOutputControl,
               int isStrucOuput,
...
//-------------------------------------------------------------------//
  ...
  struct compareDis
  {
    const ULSA4b6_DC& mySA;// data member
    compareDis(const ULSA4b6_DC& sa) : mySA(sa) {} // constructor
  ...
  bool iniDone;
  bool *aryFlagHRDone; //If it is true means Head Rotate have been done in this structure, starrt from ULSA4b6_DC
